    ...
    <strong>Title: <x-text-input type="text" name="titel" field="titel" placeholder="Title..." class="w-full border-black" autocomplete="off" :value="@old('titel')"></x-text-input> </strong> <br> <br>
    @error('title')
    <p class="text-red-500 -mt-6">{{ $message }}</p>
    @enderror

    <strong>Description:  <x-text-input type="text" name="inhoud" field="inhoud" placeholder="Description..." class="w-full border-black" autocomplete="off" :value="@old('inhoud')"></x-text-input> </strong> <br> <br>

    <strong>Device:
        <select name="tag_id" class="w-full rounded" autocomplete="off">
            <option value="">Select a Device</option>
            @foreach($tags as $tag)
                <option value="{{ $tag->id }}" {{ old('tag_id') == $tag->id ? 'selected' : '' }}>{{ $tag->naam }}</option>
            @endforeach
        </select>
    </strong> <br> <br>
    ...
    <strong>Game:
        <select name="game_id" class="w-full rounded" autocomplete="off">
            ...
            @foreach($games as $game)
                <option value="{{ $game->id }}" {{ old('game_id') == $game->id ? 'selected' : '' }}>{{ $game->naam }}</option>
            @endforeach
        </select>
    </strong> <br> <br>
    ...
    <strong>Categorie:
        <select name="categorie_id" class="w-full rounded" autocomplete="off">
            ...
            @foreach($categories as $categorie)
                <option value="{{ $categorie->id }}" {{ old('categorie_id') == $categorie->id ? 'selected' : '' }}>{{ $categorie->naam }}</option>
            @endforeach
        </select>
    </strong> <br> <br>
